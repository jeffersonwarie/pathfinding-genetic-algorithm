...
class Population {
    constructor(m, num) {
      ...
      this.matingPool = []; // ArrayList which will be used for the "mating pool"
    ...
    reproduction() {
      ...
      for (let i = 0; i < this.population.length; i++) {
        ...
        let parentAindex = int(random(this.matingPool.length));
        let parentBindex = int(random(this.matingPool.length));
        // Pick two parents
        let parentA = this.matingPool[parentAindex];
        let parentB = this.matingPool[parentBindex];
        // Get their genes
        let parentAgenes = parentA.getDNA();
        let parentBgenes = parentB.getDNA();
        // Mate their genes
        let child = parentAgenes.crossover(parentBgenes);
        // Mutate their genes
